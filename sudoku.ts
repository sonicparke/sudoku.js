...
export class Sudoku {
    ...
    BLANK_BOARD: string;
    DIGITS: string;
    SQUARE_PEERS_MAP: any;
    SQUARE_UNITS_MAP: any;
    ROWS: string;
    COLS: string;
    UNITS: any;
    ...
    generate(difficulty, unique) {
        ...
        */
        ...
        difficulty = this.forceRange(difficulty, this.NR_SQUARES + 1,
                this.MIN_GIVENS);
    ...
    solve(board, reverse?) {
        ...
        */
        ...
        let report = this.validateBoard(board);
        if (report !== true) {
            throw report;
        }
    ...
    getCandidates(board) {
        ...
        Really just a wrapper for this.getCandidatesMap for programmer
        consumption.
        */
        ...
        */
        ...
        for (let square of squares_values_map) {
            ...
            if (this.inSeq(val, this.DIGITS)) {
                let new_candidates = this.assign(candidate_map, square, val);
    ...
    eliminate(candidates, square, val) {
        ...
        */
        ...
        // with candidates.
        if (!this.inSeq(val, candidates[square])) {
            return candidates;
        }
        ...
        if (nr_candidates === 1) {
            ...
            for (let pi of this.SQUARE_PEERS_MAP[square]) {
                ...
                let candidates_new =
                        this.eliminate(candidates, peer, target_val);
        ...
        for (let ui of this.SQUARE_UNITS_MAP[square]) {
            ...
            // Otherwise the value can only be in one place. Assign it there.
                ...
                let candidates_new =
                    this.assign(candidates, val_places[0], val);
    ...
    // Conversions
    // -------------------------------------------------------------------------
    boardStringToGrid(board_string) {
